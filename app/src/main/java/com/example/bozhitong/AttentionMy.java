...
import com.example.bozhitong.fragment.adapter.AttentionMyPagerAdapter;
import com.example.bozhitong.utils.Tools;

import java.util.ArrayList;
import java.util.List;

public class AttentionMy extends FragmentActivity implements OnCheckedChangeListener {
	private ViewPager pager;
	private List<Fragment> beans;
	private AttentionMyPagerAdapter adapter;
	...
	private void initView() {
		...
		mBack=(ImageView) findViewById(R.id.img_AttentionMyback);
		mBack.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				AttentionMy.this.finish();
			}
		});
		pager = (ViewPager) findViewById(R.id.view_pager);
		adapter = new AttentionMyPagerAdapter(getSupportFragmentManager());
		...
		pager.setOnPageChangeListener(new OnPageChangeListener() {
			...
			public void onPageSelected(int position) {
				...
				RadioButton button;
				if(position==0){
					 button =(RadioButton) findViewById(R.id.tv_AttentionMy);
				}else{
					 button =(RadioButton) findViewById(R.id.tv_FansMy);
				}
